...
MY_IP = str(GET_IP_ADDRESS())                              # MY_IP contains the IP address of system
PORT = int(input("Input the peer node's port no.:"))       # Port for listening 
...
def userInput():
    while True:
        prompt = str(input("Need list of connected peers OR seeds? (Type 'peer' for peerList and 'seed' for seedList): \n")).lower()
        if(prompt== "peer"):
            print(peers_connected)
        elif(prompt == "seed"):
            print(connect_seed_addr)
